...
public sealed record TabState(string Name, List<ItemState> Items);
public sealed record WindowState(int X, int Y, int Width, int Height);
public sealed record AppState(List<TabState> Tabs, int SelectedTabIndex = 0, WindowState? Window = null);
...
public sealed class Tab(string name)
{
    ...
    public TabState ToState() => new(Name, Items.ConvertAll(i => i.ToState()));
}
...
public sealed class Board
{
    ...
    public AppState ToState(WindowState? window = null) => new(Tabs.ConvertAll(t => t.ToState()), SelectedIndex, window);
}
